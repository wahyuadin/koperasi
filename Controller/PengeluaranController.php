...
function submitSimpan($data) {
    $conn       = globalfun();
    $id_user    = $_SESSION['nasabah'];
    $id         = htmlspecialchars($data['tenor']);
    $nominal    = htmlspecialchars($data['pinjaman']);

    $query      = mysqli_query($conn, "SELECT RIGHT(id_user, 3) as kode FROM users ORDER BY id_user DESC LIMIT 1");
    $result     = $query->fetch_object();
    if ($result) { $kode = intval($result->kode) + 1; } else { $kode = 1; }
    $kodemax    = str_pad($kode, 5, rand(5, 998), STR_PAD_LEFT);
    $kodejadi   = "TRS-" . $kodemax;
    $tenor      = mysqli_fetch_object(mysqli_query($conn, "SELECT * FROM tenor WHERE id='$id'"));
    
    return mysqli_query($conn, "INSERT INTO riwayat_transaksi (id_user,id_transaksi,kategori,nominal,ket,acc) VALUES ('$id_user->id_user','$kodejadi','pengeluaran','$nominal','$id - Kredit $tenor->bulan bulan, bunga $tenor->persen %','0')");

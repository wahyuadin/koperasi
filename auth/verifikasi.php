<?php
session_start();
include(__DIR__.'/../Controller/VerifController.php');
...
if (isset($_POST['verif'])) {
    $data = [
        ...
        'kabupaten'     => htmlentities($_POST['kabupaten']),
...
<div class="container">
        ...
        <form class="needs-validation" method="POST" action="">
          <div class="row g-3">
            <div class="col-sm-6">
              ...
              <input type="text" class="form-control" name="id" placeholder="" value="<?= $_SESSION['verif']->id_user?>" required readonly>
            ...
            <div class="col-sm-6">
              ...
              <input type="text" class="form-control" name="nama" placeholder="Masukan Nama Lengkap" value="<?= $_SESSION['verif']->nama?>" required>
              <div class="invalid-feedback">
            ...
            <div class="col-md-6">
              ...
              <input type="text" class="form-control" name="rt" placeholder="000/000">
            </div>

            <div class="col-12">
              <label class="form-label">Upload KTP</label>
              <input type="file" class="form-control" name="ktp" accept=".png, .jpg" required>
              <p style="margin-left: 8px; color:red;">*Format gambar berupa JPG/PNG</p>
            </div>

          <hr class="my-4">

          <button class="w-100 btn btn-primary btn-lg" type="submit" name="verif">Kirim</button>
        </form>
      </div>
    </div>
  </main>
  ...
  <footer class="my-5 pt-5 text-body-secondary text-center text-small">
    <p class="mb-1">&copy; 2024 Koperasi Simpan Pinjam</p>
  </footer>
</div>
...
<script>
    ...
    const provinsiAPI   = baseURL + "provinces.json";
    const kabupatenAPI  = baseURL + "regencies/";
    const kecamatanAPI  = baseURL + "districts/";
    const kelurahanAPI  = baseURL + "villages/";

    function getDataFromAPI(url, callback) {
        $.getJSON(url, callback);
    }

    function fillOptions(selectId, data) {
        const select = $(`#${selectId}`);
        select.empty();
        select.append('<option selected disabled>== Pilih Salah Satu ==</option>');
    ...
    $("#provinsi").change(function () {
        ...
        getDataFromAPI(kabupatenAPI + selectedProvinsiId + ".json", function (data) {
            fillOptions("kabupaten", data);
        });
    });

    $("#kabupaten").change(function () {
        const selectedKabupatenId = $(this).val();
        getDataFromAPI(kecamatanAPI + selectedKabupatenId + ".json", function (data) {
            fillOptions("kecamatan", data);
        });
    });

    $("#kecamatan").change(function () {
        const selectedKecamatanId = $(this).val();
        getDataFromAPI(kelurahanAPI + selectedKecamatanId + ".json", function (data) {
            fillOptions("kelurahan", data);
        });
    });

    getDataFromAPI(provinsiAPI, function (data) {
        fillOptions("provinsi", data);
    });
</script>
</body>
</html>
